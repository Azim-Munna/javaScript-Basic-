...
function notifyme(){
    if(!("Notification" in window)){
        alert("this browser doesn't support");
    ...
    function notify(){
        let notification= new Notification("title of notification",{
            icon: 'http://carnes.cc/jsnuggets_avatar.jpg',
                body: "Hey! azim i am from notice",
        });
        notification.onclick=function(){
            window.open("https://www.google.com.bd")
        };
...
console.log("Immediately Invoked Function area ");
(function(){
    console.log("my faborite number 2018");
})();
(favN=function(num=3){
    console.log("my fabrate num is " + num);
})();
favN(2946);
...
function xll(){
    "use strict";
   let xml=5 // if i dont use vr name it will be show arror :P 
   c= xml*2;
   console.log(c);
}
xll()
const obj2 ={}; 
Object.defineProperty(obj2,"property_one",{
    value:45,
    writable:false
});
obj2.property_one=71;
console.log(obj2.property_one);

// check if property
var check_if={
    name:"i am azim munna"
};
if(check_if.name){
    console.log("check if checked ");
}
console.log(check_if.hasOwnProperty("name"));
console.log("name" in check_if);
// timeout 

let clert=setTimeout(bye,3000);
clearTimeout(clert);
function bye(){
    console.log("hi i'm form timeout so good bye ");
}
console.log("time out ");

var count = 0;
var intid= setInterval(counters,1000);
function start(){
...
function counters(){
 ...
 let time_on=0;
 function timecount(){
    document.getElementById("timer").value=wo_c;
    wo_c=wo_c+1;
    wo_t=setTimeout(timecount,1000);
 }
...
clearTimeout(wo_t);
 ...
 // from w3
function try_fun(){
let trye=document.getElementById("try_input").value;
let try_shw_data = document.getElementById("try_data");
try_shw_data.innerHTML="";
    try {
        if(trye=="")  throw "empty";
        if(isNaN(trye)) throw "not a number";
        trye =Number(trye);
        if(trye<5)  throw "too low";
        if(trye <=9) throw "ok"
            
        if(trye>=10) throw " too high"             
    }
    catch(ass){
        try_shw_data.innerHTML="input is " +ass;
        console.log("input is " +ass);
    }
...
let f_json='{"age":30}';
try{
    let f_user = JSON.parse(f_json);
    if(!f_json.name){
        throw new SyntaxError("incomlete there is no name ");
    }
    console.log(f_json.name);
}
catch(er){
    console.log("json error "+er.message); // if you use syntaxError maybe you have to use message 
}
// date 
console.log(" date area ");

let date_f = new Date();
// console.log(date_f.getDate());
let det_st= date_f.toTimeString();
// let det_dt= new Date(2017,1,3,42,43,23,23);
console.log(det_st);
// console.log(det_dt);
// date_s.innerHTML=det_dt;
date_d.innerHTML=det_st;
// making date function
let startss= new Date();
dosomething();
let end= new Date();
let elapsed=end.getTime()-startss.getTime();
console.log(elapsed);
function dosomething(){
    for(var ml=0; ml<10000000; ml++) {
